...
const ProjectSection = async () => {
  ...
  const featured = feat?.data.map((item: any, i: number) => {
    const slug = `${item.attributes.name.toLowerCase().replace(' ', '-')}-at-${item.attributes.location.city
      .toLowerCase()
      .replace(' ', '-')}`;
    const classes = i % 2 ? 'w-full md:w-3/5 mt-10s md:mt-10s' : 'w-full md:w-1/3 mt-4s md:-mt-24s h-fit';
    return (
  ...
  return (
    <section className='w-4/5 mx-auto my-6 md:my-32'>
      ...
      <div className='row flex flex-col md:flex-rows justify-between mb-4'>{featured}</div>
